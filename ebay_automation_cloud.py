...
EBAY_CLIENT_ID = st.secrets.get("EBAY_CLIENT_ID", os.getenv("EBAY_CLIENT_ID", ""))
...
def login_screen() -> None:
        ...
        <div class="login-card">
            <div class="brand">
                ...
                <div>
                    ...
                    <div class="brand-subtitle">Premium seller operations dashboard</div>
                </div>
            </div>
        """,
        unsafe_allow_html=True,
    ...
    with tab_login:
        ...
        st.caption("Default admin is created on first run. Change its password immediately.")

    with tab_signup:
        new_user = st.text_input("Email / Username", key="signup_user")
    ...
    with tab_forgot:
        ...
        if st.button("Reset password", use_container_width=True):
            if (
                st.session_state.get("reset_user") == email
                and st.session_state.get("reset_otp") == otp
                and time.time() - st.session_state.get("reset_otp_time", 0) < 900
            ):
                users = load_users()
    ...
    st.markdown("</div>", unsafe_allow_html=True)
    st.markdown(
        '<div class="footer">eBay-only automation • Secure credential configuration • v2.0</div>',
        unsafe_allow_html=True,
    )
...
def get_fresh_token(store: Dict[str, Any], production: bool = True) -> Optional[str]:
...
""",
    ...
    if st.button("Save connected store", type="primary", use_container_width=True):
        if not code or not alias:
            st.warning("Enter authorization code and store alias.")
            return
